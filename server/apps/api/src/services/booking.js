...
export class BookingService {
  ...
  static async createBooking(bookingData) {
    ...
    try {
      ...
      if (availability.length === 0) {
        ...
        return {
          ...
          error: {
            ...
            status: 400,
            details: [{ field: 'preferredDate', message: 'No availability slots for this date' }]
          }
        };
      }
      ...
      logger.info('Booking created successfully', { bookingId, participants, dateStr });
  ...
  static async updateBookingStatus(id, newStatus) {
    ...
    try {
      await connection.beginTransaction();
      logger.debug('Started status update transaction', { bookingId: id, newStatus });
      ...
      await connection.commit();
      logger.info('Booking status updated', { bookingId: id, oldStatus, newStatus });
      ...
      await connection.rollback();
      logger.error('Failed to update booking status', { bookingId: id, newStatus, error });
      throw error;
    } finally {
      connection.release();
    }
