...
export const validateBookingInput = (input) => {
  ...
  if (!input || typeof input !== 'object') {
    return { isValid: false, errors: [{ field: 'root', message: 'Invalid input data' }] };
  }

  if (!input.name || typeof input.name !== 'string' || input.name.trim().length < 2 || input.name.trim().length > 255) {
  ...
  if (!input.phone || typeof input.phone !== 'string' || input.phone.trim().length < 8 || input.phone.trim().length > 50) {
    errors.push({ field: 'phone', message: 'Phone number must be between 8 and 50 characters' });
  }
  ...
  if (!input.participants || typeof input.participants !== 'number' || input.participants < 1 || input.participants > 20) {
    errors.push({ field: 'participants', message: 'Participants must be between 1 and 20' });
  }

  if (!input.bookingType || !['dive', 'course', 'discover', 'advanced'].includes(input.bookingType)) {
    errors.push({ field: 'bookingType', message: 'Valid booking type is required' });
  }

  if (input.specialRequests && typeof input.specialRequests === 'string' && input.specialRequests.length > 2000) {
    errors.push({ field: 'specialRequests', message: 'Special requests must not exceed 2000 characters' });
  }
...
export const sanitizeBookingInput = (input) => {
  ...
  const sanitized = {
    ...
    specialRequests: input.specialRequests ?
      ...
      (input.specialRequests === null || input.specialRequests === undefined) ? input.specialRequests : '',
...
export const validateBookingBusinessRules = (bookingData) => {
  ...
  if (!bookingData || !bookingData.preferredDate) {
    return { isValid: false, errors: [{ field: 'preferredDate', message: 'Preferred date is required' }] };
  }
  ...
  if (['dive', 'discover', 'advanced'].includes(bookingData.bookingType) && !bookingData.diveSiteId) {
    errors.push({
      field: 'diveSiteId',
