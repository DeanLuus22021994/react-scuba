...
export const sanitizeBookingInput = (input) => {
	...
	const sanitized = {
		...input,
		name: input.name
			? stripHTML(String(input.name)).trim().replace(/\s+/g, " ")
			: "",
		email: input.email
			? stripHTML(String(input.email)).trim().toLowerCase()
			: "",
		...
		participants: input.participants
			? Math.floor(Number(input.participants))
			: 1,
...
export const validateBookingBusinessRules = (bookingData) => {
	...
	if (!bookingData || !bookingData.preferredDate) {
		return {
			isValid: false,
			errors: [
				{ field: "preferredDate", message: "Preferred date is required" },
			],
		};
	}
